...
$(function(){
    ...
    tinymce.init({
        ...
        selector: '#sentcCn', // 내 거에 맟춰서 변경 id가 boardCn인 곳을 찾아라 
        height: 500,
        menubar: false,
        plugins: plugins,
...
<c:choose>
	<c:when test="${not empty searchVO.sentcId}">
		<c:set var="actionUrl" value="/sentc/update.do"/>
	</c:when>
	<c:otherwise>
		<c:set var="actionUrl" value="/sentc/insert.do"/>
	</c:otherwise>
</c:choose>
...
<div id="contents">
	...
	<form action="${actionUrl}" method="post" id="frm" name="frm" onsubmit="return regist()" enctype="multipart/form-data">
		...
		<input type="hidden" name="sentcId" value="${result.sentcId}"/>
		<input type="hidden" name="returnUrl" value="/sentc/sentcRegist.do"/>
		...
		<table class="chart2">
			...
			<tbody>
				<tr>
					...
					<td>
						<input type="text" id="sentcSj" name="sentcSj" title="제목입력" class="q3" value="<c:out value="${result.sentcSj}"/>"/>
					</td>
				</tr>
				<tr>
				...
				<tr>
					<th scope="row">내용</th>
					<td>
						<textarea id="sentcCn" name="sentcCn" rows="15" title="내용입력"><c:out value="${result.sentcCn}"/></textarea>
		...
		<div class="btn-cont ar">
			...
			<c:url var="listUrl" value="/sentc/selectList.do"/>
...
function regist(){
	...
	$("#sentcCn").val(tinymce.activeEditor.getContent());
